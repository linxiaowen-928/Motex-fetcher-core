...
export class StorageService extends Service {
  ...
  constructor(ctx: Context, config: StorageConfig) {
    super(ctx, 'storage')
    this.outDir = join(process.cwd(), config.outDir)
  ...
  async append(item: ParsedItem): Promise<void> {
    ...
    await this.writeLine(`${out.source}.jsonl`, out)
    try {
      await appendFile(join(this.outDir, `${out.source}.done.urls`), out.url + '\n', 'utf-8')
  ...
  async saveBinary(res: FetchResponse): Promise<void> {
    ...
    await mkdir(join(this.outDir, res.source), { recursive: true })
    let ext = 'bin'
    try {
    ...
    const file = `${sha}.${ext}`
    const dir = join(this.outDir, res.source)
    await writeFile(join(dir, file), Buffer.from(res.body))
    await appendFile(join(this.outDir, `${res.source}.meta.jsonl`),
  ...
  async downloadRawFiles(urls: string[], source: string, _chunks = 8): Promise<{ ok: number; skipped: number; failed: number }> {
    ...
    const resultPath = join(this.outDir, '.worker_result.json')
    const inputPath = join(this.outDir, '.worker_input.json')
    await mkdir(this.outDir, { recursive: true })
    await writeFile(inputPath, JSON.stringify({ urls, source, outDir: this.outDir, resultPath }), 'utf-8')
  ...
  private async writeLine(file: string, obj: unknown) {
    await mkdir(this.outDir, { recursive: true })
    const shard = this.currentShard(file)
    await appendFile(join(this.outDir, shard), JSON.stringify(obj) + '\n', 'utf-8')
    this.ctx.logger.debug('saved %s', shard)
  }

  /** 分片选择：主文件 <file> 超限 → 找 .part1/.part2/...（各分片也超限则继续递增） */
  private currentShard(file: string): string {
    const SHARD = 64 * 2 ** 20
    const p = join(this.outDir, file)
    if (existsSync(p) && statSync(p).size >= SHARD) {
      let n = 1
      for (;;) {
        const sp = join(this.outDir, `${file}.part${n}`)
